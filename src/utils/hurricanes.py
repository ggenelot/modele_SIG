...
def filter_track_in_geometry(
    ...
    def check_hurricane(group):
        points = gpd.GeoSeries(
            [Point(lon, lat) for lon, lat in zip(group[lon_col], group[lat_col])]
        )
...
def filter_track(
    df, column, condition, tc_col="TC number", year_col="Year", return_filtered=True
):
...
def polygon_from_point_radius(lat_center, lon_center, radius_km, n_points=64):
    ...
    coords_latlon = [
        _destination_point(lat_center, lon_center, b, radius_km) for b in bearings
    ]
...
def track_to_ds(track, resolution=0.05):
    ...
    for i, step in track.iterrows():
        ...
        wind_speed = step["Maximum wind speed"] * mask_points_within_distance(
            da, step["Latitude"], step["Longitude"], step["Radius to maximum winds"]
        )
